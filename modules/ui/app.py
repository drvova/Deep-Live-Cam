...
img_ft, vid_ft = config.file_types
...
def save_switch_states():
    switch_states = {
        ...
        "many_faces": config.many_faces,
        "map_faces": config.map_faces,
        "color_correction": config.color_correction,
        "nsfw_filter": config.nsfw_filter,
        "live_mirror": config.live_mirror,
        "live_resizable": config.live_resizable,
        "fp_ui": config.fp_ui,
        "show_fps": config.show_fps,
        "mouth_mask": config.mouth_mask,
        "show_mouth_mask_box": config.show_mouth_mask_box,
    }
...
def load_switch_states():
    try:
        ...
        config.keep_fps = switch_states.get("keep_fps", True)
        config.keep_audio = switch_states.get("keep_audio", True)
        config.keep_frames = switch_states.get("keep_frames", False)
        config.many_faces = switch_states.get("many_faces", False)
        config.map_faces = switch_states.get("map_faces", False)
        config.color_correction = switch_states.get("color_correction", False)
        config.nsfw_filter = switch_states.get("nsfw_filter", False)
        config.live_mirror = switch_states.get("live_mirror", False)
        config.live_resizable = switch_states.get("live_resizable", False)
...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    keep_fps_value = ctk.BooleanVar(value=config.keep_fps)
    keep_fps_checkbox = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "keep_fps", keep_fps_value.get()),
            save_switch_states(),
        ),
    )
    keep_fps_checkbox.place(relx=0.1, rely=0.5)

    keep_frames_value = ctk.BooleanVar(value=config.keep_frames)
    keep_frames_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "keep_frames", keep_frames_value.get()),
            save_switch_states(),
        ),
    )
    keep_frames_switch.place(relx=0.1, rely=0.55)

    enhancer_value = ctk.BooleanVar(value=config.fp_ui["face_enhancer"])
    ...
    keep_audio_value = ctk.BooleanVar(value=config.keep_audio)
    keep_audio_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "keep_audio", keep_audio_value.get()),
            save_switch_states(),
        ),
    )
    keep_audio_switch.place(relx=0.6, rely=0.5)

    many_faces_value = ctk.BooleanVar(value=config.many_faces)
    many_faces_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "many_faces", many_faces_value.get()),
            save_switch_states(),
        ),
    )
    many_faces_switch.place(relx=0.6, rely=0.55)

    color_correction_value = ctk.BooleanVar(value=config.color_correction)
    color_correction_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "color_correction", color_correction_value.get()),
            save_switch_states(),
        ),
    )
    color_correction_switch.place(relx=0.6, rely=0.6)

    #    nsfw_value = ctk.BooleanVar(value=config.nsfw_filter)
    #    nsfw_switch = ctk.CTkSwitch(root, text='NSFW filter', variable=nsfw_value, cursor='hand2', command=lambda: setattr(config, 'nsfw_filter', nsfw_value.get()))
    #    nsfw_switch.place(relx=0.6, rely=0.7)

    map_faces = ctk.BooleanVar(value=config.map_faces)
    map_faces_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "map_faces", map_faces.get()),
            save_switch_states(),
    ...
    show_fps_value = ctk.BooleanVar(value=config.show_fps)
    show_fps_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(config, "show_fps", show_fps_value.get()),
            save_switch_states(),
        ),
    )
    show_fps_switch.place(relx=0.6, rely=0.65)

    mouth_mask_var = ctk.BooleanVar(value=config.mouth_mask)
    mouth_mask_switch = ctk.CTkSwitch(
        root,
        text=_("Mouth Mask"),
        variable=mouth_mask_var,
        cursor="hand2",
        command=lambda: setattr(config, "mouth_mask", mouth_mask_var.get()),
    )
    mouth_mask_switch.place(relx=0.1, rely=0.45)

    show_mouth_mask_box_var = ctk.BooleanVar(value=config.show_mouth_mask_box)
    ...
    def on_transparency_change(value: float):
        ...
        val = float(value)
        config.opacity = val  # Set global opacity
        ...
        if percentage == 0:
            ...
            config.face_swapper_enabled = True
...
def analyze_target(start: Callable[[], None], root: ctk.CTk):
    ...
    if config.map_faces:
        ...
        if len(config.source_target_map) > 0:
            create_source_target_popup(start, root, config.source_target_map)
        else:
...
def select_source_path() -> None:
    ...
    if is_image(source_path):
        ...
        RECENT_DIRECTORY_SOURCE = os.path.dirname(config.source_path)
        image = render_image_preview(config.source_path, (200, 200))
...
def swap_faces_paths() -> None:
    ...
    RECENT_DIRECTORY_SOURCE = os.path.dirname(config.source_path)
    RECENT_DIRECTORY_TARGET = os.path.dirname(config.target_path)

    PREVIEW.withdraw()

    source_image = render_image_preview(config.source_path, (200, 200))
    source_label.configure(image=source_image)

    target_image = render_image_preview(config.target_path, (200, 200))
...
def select_target_path() -> None:
    ...
    if is_image(target_path):
        ...
        RECENT_DIRECTORY_TARGET = os.path.dirname(config.target_path)
        image = render_image_preview(config.target_path, (200, 200))
...
def select_output_path(start: Callable[[], None]) -> None:
    ...
    if output_path:
        ...
        RECENT_DIRECTORY_OUTPUT = os.path.dirname(config.output_path)
        start()
...
def toggle_preview() -> None:
    if PREVIEW.state() == "normal":
        PREVIEW.withdraw()
    elif config.source_path and config.target_path:
        init_preview()
        update_preview()
...
def init_preview() -> None:
    ...
    if is_video(config.target_path):
        video_frame_total = get_video_frame_total(config.target_path)
...
def webcam_preview(root: ctk.CTk, camera_index: int):
    ...
    if not config.map_faces:
        ...
        create_source_target_popup_for_webcam(
            root, config.source_target_map, camera_index
        )
...
def create_webcam_preview(camera_index: int):
    ...
    while True:
        ...
        if not config.map_faces:
            if source_image is None and config.source_path:
                source_image = get_one_face(cv2.imread(config.source_path))

            for frame_processor in frame_processors:
                if frame_processor.NAME == "DLC.FACE-ENHANCER":
                    if config.fp_ui["face_enhancer"]:
