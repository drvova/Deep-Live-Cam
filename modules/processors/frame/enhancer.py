"""Face enhancer processor for Deep-Live-Cam."""
...
from modules.face.analyser import get_one_face
...
def get_face_enhancer() -> Any:
    ...
    with THREAD_LOCK:
        if FACE_ENHANCER is None:
            ...
            try:
                ...
                # Fallback to CPU if initialization with GPU fails
                if device is not None and device.type != "cpu":
                    ...
                    try:
                        ...
                        print(
                            f"{NAME}: GFPGANer initialized successfully on CPU after fallback."
                        )
                    except Exception as fallback_e:
                        print(
                            f"{NAME}: FATAL: Could not initialize GFPGANer even on CPU: {fallback_e}"
                        )
                        FACE_ENHANCER = None
                else:
    ...
    if FACE_ENHANCER is None:
        raise RuntimeError(
            f"{NAME}: Failed to initialize GFPGANer. Check logs for errors."
        )
...
def process_frames(
    source_path: Optional[str], temp_frame_paths: List[str], progress: Any = None
) -> None:
...
def process_image(
    source_path: Optional[str], target_path: str, output_path: str
) -> None:
...
def process_video(source_path: Optional[str], temp_frame_paths: List[str]) -> None:
    """Processes video frames using the frame processor registry."""
    registry.process_video(source_path, temp_frame_paths, process_frames)
